...
class Validator(BaseModel, ABC):
    ...
    @abstractmethod
    def validate_retrieved_source(self) -> bool:
        pass
...
class RecallOverlap(Validator):
    def validate_retrieved_source(self) -> bool:
        ...
        if truth_end == truth_start:
            print(f"{self.ground_truth_path.file_path} has not same start and stop")
            return True
...
class SingleAnswerValidator(BaseModel):
    ...
    def validate_answer(self) -> bool:
        ...
        ValidatorGuards.question_comparison(ground_truth.question, self.answer.question)

        recall_instance = self.recall(
            n=self.n,
            ground_truth_path=ground_truth.sources[0],
            indexer_retrieved_paths=self.answer.retrieved_sources
            )
...
class BatchAnswerValidator(BaseModel):
    ...
    def validate_answer(self) -> List[bool]:
        results: List[bool] = []
        for answer in self.answers:
            try:
                single_validator_instance = SingleAnswerValidator(
                    ground_truth_map=self.ground_truth_map,
                    answer=answer,
                    recall=self.recall,
                    n=self.n
                )
                results.append(single_validator_instance.validate_answer())
